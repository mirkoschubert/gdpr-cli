...
class JSParser {
  ...
  hasGoogleAnalytics() {
    ...
    this.js.inline.forEach(js => {
      if (js.indexOf('google-analytics.com/analytics.js') !== -1) hasAnalytics = 'standard';
    });
    ...
    // Check for legacy ga.js snippet (deprecated)
    // TODO: Legacy ga.js snippet
    this.js.inline.forEach(js => {
      if (js.indexOf('google-analytics.com/ga.js') !== -1) hasAnalytics = 'legacy';
    });
    return hasAnalytics;
  }
  ...
  getGoogleAnalyticsFeatures() {
    ...
    if (this.hasGoogleAnalytics() !== 'none') {
      this.js.inline.forEach(js => {
        const hasStandardGA = js.match(/(\(\'create)(.*)(\))/) !== null;
        const hasLegacyGA = js.indexOf('_gaq.push') !== -1;

        if (hasStandardGA || hasLegacyGA) entry.property_id = js.match(/UA\-[0-9]*\-[0-9]/)[0];
        if (hasStandardGA) entry.anonymize_ip = js.indexOf('\'anonymizeIp\', true') !== -1;
        if (hasStandardGA) entry.force_ssl = js.indexOf('\'forceSSL\', true') !== -1;
        entry.type = this.hasGoogleAnalytics();
      });
      return entry;
    } else return;
  }
