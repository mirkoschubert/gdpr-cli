...
class Tasks {
  ...
  getFontInformation() {
    if (this.hasTask('fonts')) {
      this.data.fonts = this.cp.getFonts();
      if (this.jp.hasTypekitJS()) this.data.fonts.typekit = this.jp.getTypekitJSFonts();
      ...
      if (Object.keys(this.data.fonts).length === 0) {
        this.ui.info('There were no Fonts found.');
        //console.log(this.data.fonts);
      } else {
        if (typeof this.data.fonts.google !== 'undefined') {
          const gf = this.cp.countFonts(this.data.fonts.google);
          this.ui.info('Google Fonts loaded: ' + chalk.red(gf.length) + '\n');
          gf.forEach((f, i) => {
            this.ui.tableitem([
              chalk.yellow(String(i + 1) + '.'), f,
              chalk.dim(this.cp.getFontStyles(this.data.fonts.google, f))
            ]);
          });
        }
        if (this.jp.hasTypekitJS()) {
          const tk = this.cp.countFonts(this.data.fonts.typekit);
          this.ui.info('Adobe TypeKit Fonts loaded: ' + chalk.red(tk.length) + '\n');
          tk.forEach((f, i) => {
            this.ui.tableitem([chalk.yellow(String(i + 1) + '.'), f, chalk.dim(this.cp.getFontStyles(this.data.fonts.typekit, f))]);
          });
        }
        if (typeof this.data.fonts.wordpress !== 'undefined') {
          const wf = this.cp.countFonts(this.data.fonts.wordpress);
          if (typeof this.data.fonts.google !== 'undefined') console.log('');
          this.ui.info(
            'Fonts from wordpress.com loaded: ' + chalk.red(wf.length) +
            '\n');
          wf.forEach((f, i) => {
            this.ui.tableitem([
              chalk.yellow(String(i + 1) + '.'), f,
              chalk.dim(this.cp.getFontStyles(this.data.fonts.wordpress, f))
            ]);
          });
        }
        if (typeof this.data.fonts.internal !== 'undefined') {
          const intf = this.cp.countFonts(this.data.fonts.internal);
          if (typeof this.data.fonts.wordpress !== 'undefined') console.log('');
          this.ui.info(
            'Fonts directly from the site: ' + chalk.red(intf.length) + '\n');
          intf.forEach((f, i) => {
            this.ui.tableitem([
              chalk.yellow(String(i + 1) + '.'), f,
              chalk.dim(this.cp.getFontStyles(this.data.fonts.internal, f))
            ]);
          });
        }
        if (typeof this.data.fonts.inline !== 'undefined') {
          const inlf = this.cp.countFonts(this.data.fonts.inline);
          if (typeof this.data.fonts.iternal !== 'undefined') console.log('');
          this.ui.info(
            'Inline Fonts from the CSS: ' + chalk.red(inlf.length) + '\n');
          inlf.forEach((f, i) => {
            this.ui.tableitem([
              chalk.yellow(String(i + 1) + '.'), f,
              chalk.dim(this.cp.getFontStyles(this.data.fonts.inline, f))
            ]);
          });
        }
      }
