...
class Tasks {
    ...
    if (this.hasTask('prefetching')) {
      ...
      if (dns.length === 0) {
        ...
        dns.forEach((url, i) => {
          ...
          if (u.match(/s\.w\.org/) !== null) explanation = '  (WordPress Update Service)';
          if (u.match(/s[0-9]\.wp\.com/) !== null) explanation = '  (WordPress CDN - Styles)';
          if (u.match(/i[0-9]\.wp\.com/) !== null) explanation = '  (WordPress CDN - Images)';
          if (u.match(/v[0-9]\.wordpress\.com/) !== null) explanation = '  (WordPress CDN - Videos)';

          this.ui.info(chalk.yellow(String(i) + '.') + ' ' + u + chalk.dim(explanation));
        });
      }
  ...
  getFontInformation() {
    if (this.hasTask('fonts')) {
      ...
      this.ui.info('This Section is coming soon.')
      this.remove('fonts');
    }
  }
