...
class Tasks {
    ...
    if (this.hasTask('prefetching')) {
      ...
      if (dns.length === 0) {
        ...
        dns.forEach((url, i) => {
          ...
          if (u.match(/use\.fontawesome\.com/) !== null) explanation = 'Font Awesome CDN';
  ...
  getGeneralInformation() {
    ...
    if (gen.toLowerCase().indexOf('wordpress') !== -1 || gen.toLowerCase().indexOf('wpml') !== -1) {
      let theme = '';
      this.data.css.forEach(css => {
