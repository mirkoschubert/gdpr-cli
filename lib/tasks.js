...
class Tasks {
  ...
  getSocialMediaInformation() {
    ...
    if (this.hasTask('social')) {
      ...
      for ( var sm in this.data.social ) {
        ...
        this.recommendations.add('Social medias processors', sm);
      }
  ...
  getCookiesInformation() {
    ...
    for ( var i in cookies ) {
      ...
      if ( t1 > t2 ) {
        this.recommendations.add('Cookies', 'lifetime');
  ...
  getNFZInformations() {
    ...
    this.ui.listitem('Web cache', 'Voided');
    this.ui.listitem('Web cookies', 'Voided');
    ...
    dns.resolve(tld, 'A', function(err, addresses){
      if ( err !== null ) return;
      tasks.ui.listitem('Website IPv4', addresses);
    });
    dns.resolve(tld, 'AAAA', function(err, addresses){
      if ( err !== null ) return;
      tasks.ui.listitem('Website IPv6', addresses);
    });
    dns.resolve('resolver1.opendns.com', 'A', function(err, addresses){
      ...
      dns.resolve('myip.opendns.com', 'A', function(err, addresses){
        if ( err !== null ) return;
        tasks.ui.listitem("Auditor's IPv4", addresses);
      });
      dns.resolve('myip.opendns.com', 'AAAA', function(err, addresses){
        if ( err !== null ) return;
        tasks.ui.listitem("Auditor's IPv6", addresses);
      });
    });
<<<<<<< HEAD
=======
  ...
  getAuditInformations() {
    if (!this.hasTask('audit')) {
        console.error('no recommendation expected');
        return;
    }
...
>>>>>>> b038e2a... Going further into NF Z67-147 + preparation of further auditing
  }
