...
class Tasks {
  ...
  getCookiesInformation() {
    ...
    if (typeof this.data.html.headers['set-cookie'] == 'object') {
      cookies.push(this.data.html.headers['set-cookie']);
    }
    ...
    [this.data.js, this.data.css].forEach(type => {
      type.forEach(data => {
        if (typeof data.headers['set-cookie'] == 'object') {
          cookies.push(data.headers['set-cookie']);
        }
      });
    });
    ...
    var getter = new Cookies(cookies);
    ...
    for ( var i in cookies ) {
      ...
      if ( t1 > t2 ) {
        this.ui.error('Cookie "'+cookies[i].name+'" expires in more than '+this.args.cookies+' month (expires on '+cookies[i].expires.toLocaleDateString()+')', false);
        reco = true;
        continue;
      }
