...
class CSSParser {
  ...
  hasLocalFonts(localUrl) {
    let lf = false;
    this.css.forEach(el => {
      if (el.url.host === localUrl.host && el.content.indexOf('@font-face')) lf = true;
    });
        ...
        if (fontfaces !== null) {
          fontfaces.forEach(ff => {
            ...
            if (isLocal) {
              ...
              if (ff.match(/font\-style\:\s?['"]?([^'";]*)/) !== null) font.style = ff.match(/font\-style\:\s?['"]?([^'";]*)/)[1];
              if (ff.match(/font\-weight\:\s?['"]?([^'";]*)/) !== null) font.weight = ff.match(/font\-weight\:\s?['"]?([^'";]*)/)[1];
  ...
  hasWordPressFonts() {
    let wf = false;
    this.css.forEach(el => {
      if (el.url.href.match(/s[0-9]?\.wp\.com/) !== null && el.content.match(/\@font\-face/)) wf = true;
    });
    return wf;
  }
        ...
        if (fontfaces !== null) {
          ...
          fontfaces.forEach(ff => {
            ['Noticons', 'Genericons', 'social-logos'].forEach(f => {
              if (ff.indexOf(f) !== -1 && JSON.stringify(fonts).indexOf(f) === -1) fonts.push({
                font: f,
                source: 'wordpress'
              });
            });
